...
public class IDMaker {
    public static long IDMake(){
        int temp=0;
        long timeMillis = System.currentTimeMillis();
        temp= (int) timeMillis;
        Random random=new Random();
        long randomHead=random.nextInt();
        randomHead<<=32;
        long res= randomHead|temp;
        return  res;
    }
     ...
     * @return
     */
    public static List<User> getName_Sex_age(int size) {
       List<User> res=new ArrayList<>(size);
        for (int i = 0; i < size; i++) {
            res.add(getSingleUser());
        }
        return res;
    }

    private static User getSingleUser(){
        String sex=getSex();
        int randNum = new Random().nextInt(2) + 1;
        ...
        User res=new User(IDMake(),name,sex,getAge(),"123456");
        return  res;
    }
